...
from module.latent import Latent_Bert_H, Latent_Bert_T
...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, targets, alpha=0, train=True):
        ...
        if train:
            ...
            hateful_image_embedding, twitter_image_embedding = image_embed.chunk(2, 0)
            hateful_text_embedding, twitter_text_embedding = output.hidden_states[6][:,0,:].chunk(2, 0)
            hateful_i, hateful_t = self.dis_H(hateful_image_embedding, hateful_text_embedding)
            twitter_i, twitter_t = self.dis_T(twitter_image_embedding, twitter_text_embedding)
